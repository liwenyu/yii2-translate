...
- ✅ 支持HTML标签前的货币单位替换（如 `:{user_gift}元<` 会被正确替换）
...
- `'元'` → `'￥'`（单独的货币单位）
- `':{user_gift}元<'` → `':{user_gift}￥<'`（HTML标签前的货币单位）
- `'100元>'` → `'100￥>'`（HTML标签前的货币单位）
...
3. **支持HTML标签前的货币单位**：如果货币单位后面是HTML标签（如 `<`、`>`），会被正确替换（如 `:{user_gift}元<` → `:{user_gift}￥<`）
4. **精确匹配**：组件会精确匹配独立的货币单位，例如：
   - ✅ `'100元'` → `'100￥'`（会被替换）
   - ✅ `'元'` → `'￥'`（会被替换）
   - ✅ `':{user_gift}元<'` → `':{user_gift}￥<'`（会被替换）
   - ❌ `'元素'` → `'元素'`（不会被替换，因为"元"后面是"素"）
5. 如果 `Yii::$app->params['currency_symbol']` 不存在，将使用 `defaultCurrencySymbol` 的值
6. 可以通过设置 `enableCurrencyReplace` 为 `false` 来禁用货币替换功能
7. 组件继承自 `yii\i18n\PhpMessageSource`，支持所有父类的功能和配置选项
...
### Q: HTML标签前的货币单位（如 `:{user_gift}元<`）会被替换吗？

A: 是的！从 1.1.2 版本开始，组件支持替换HTML标签前的货币单位。例如：

```php
// messages/zh-CN/app.php
return [
    'gift_message' => ':{user_gift}元<',
];
```

当调用 `Yii::t('app', 'gift_message')` 时，会自动将 `'元'` 替换为配置的货币符号（如 `'￥'`），最终输出 `':{user_gift}￥<'`。
...
- 🐛 **修复**：修复HTML标签前货币单位无法被替换的问题
  - 修复正则表达式，在前瞻断言中添加 `<` 和 `>` 符号
  - 现在可以正确匹配 `:{user_gift}元<` 这种情况
- ✅ **测试**：添加测试用例验证HTML标签场景的货币单位替换
